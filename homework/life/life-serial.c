...
void simulate_life_serial(LifeBoard *state, int steps) {
    LifeBoard *odd_board = LB_new(state->width, state->height);
    ...
    for (int step = 0; step < steps; step += 1) {
        ...
        /* now that we computed odd_board, make it the current state */
        LB_swap(state, odd_board);
    }
    LB_del(odd_board);
}
